...
void generate_terrain(Grid &grid) {
    for (int x = 0; x < 64; ++x) {
        for (int z = 0; z < 64; ++z) {
            float height_val =
                14.0f + 8.0f * std::sin(x * 0.15f) * std::cos(z * 0.15f) +
                4.0f * std::sin((x + z) * 0.1f);
    ...
    auto add_sphere = [&](int cx, int cy, int cz, int r, uint8_t cr, uint8_t cg,
                          uint8_t cb) {
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *window =
        SDL_CreateWindow("ExpVoxel | [SPACE] Toggle Auto-Orbit | [WASD] Move | "
                         "[R] Reset Terrain",
    ...
    std::thread mutator_thread([&grid, &sim_running] {
        ...
        while (sim_running.load(std::memory_order_relaxed)) {
            ...
            if (prev_px >= 0) {
                grid.clear_voxel_atomic(prev_px, prev_py, prev_pz);
            }
    ...
    while (running) {
        ...
        Vec3 cam_right =
            Vec3{cam_forward.y * world_up.z - cam_forward.z * world_up.y,
                 cam_forward.z * world_up.x - cam_forward.x * world_up.z,
                 cam_forward.x * world_up.y - cam_forward.y * world_up.x}
                .normalized();
        Vec3 cam_up =
            Vec3{cam_right.y * cam_forward.z - cam_right.z * cam_forward.y,
        ...
        auto start_time = std::chrono::high_resolution_clock::now();
        {
            ...
            render_tiles_multithreaded(thread_pool, grid, pixels.data(),
                                       RENDER_WIDTH, RENDER_HEIGHT, cam_pos,
                                       fov, cam_forward, cam_right, cam_up);
        }
        ...
        std::string title =
            "ExpVoxel | Threads: " +
            std::to_string(thread_pool.thread_count()) +
